...
上に書いたのは、ひとつの表現ですが、AI[^01-intro-3] にも表題の「データサイエンスとは」と聞いてみると、次のような答えをしてくれました。
