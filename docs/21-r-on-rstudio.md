...
注意：以下は、自分のコンピュータ、すなわち、管理者権限があるコンピュータにインストールすることを前提に書きます。
...
漢字やカタカナが現れないのにパッケージなどがインストールできず、エラーが出たら、まず、RStudio の上のメニューの、Tools の一番下の、Global Options を開き、左のメニューから、Packages を選び、Primary CRAN Repository を Change として、CRAN mirror から、Japan: The Institute of Statistical Mathematics, Tokyo を選択してください。これで解決することもあるようです^[理由は不明ですが、特定のミラーサイトが理由ではなく、https を能動的に設定することで解決が図れるのかなと思います。]。
...
Windows で、インストール時、または、その後の、パッケージのインストールで問題が起こった場合の対処法を書きます。
